...
export default async function CitizenReturneesDashboardPage({ searchParams }: Props) {
  const s = await auth();
  if (!s?.user) redirect("/citizen/login?next=/services/returnees");
    ...
    <div className="gov-panel w-full min-w-0 max-w-full px-0">
      ...
      <div className="gov-card p-4 md:p-6">
        <ReturneeRegistrationForm
          ...
          successReturnPath="/services/returnees"
          prefill={{
            name: prefill?.name,
            phone: prefill?.phone,
