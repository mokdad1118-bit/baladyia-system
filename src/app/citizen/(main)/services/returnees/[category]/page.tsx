...
export default async function CitizenSocialServiceCategoryPage({ params, searchParams }: Props) {
  const s = await auth();
  ...
  if (!category) redirect("/citizen/services/returnees");
  ...
  return (
    <div className="gov-panel w-full px-0">
      <header className="gov-page-heading mb-6 border-b border-[var(--gov-border)] pb-4">
        <h1 className="text-lg font-bold text-[var(--gov-text)] md:text-xl">{socialServiceCategoryLabelAr[category]}</h1>
      </header>
      ...
      <div className="gov-card p-4 md:p-6">
        <SocialServiceCaseForm category={category} action={submitSocialServiceCase} successReturnPath="/citizen/services/returnees" prefill={{ name: prefill?.name, phone: prefill?.phone, nationalId: prefill?.nationalId, email: emailDefault || null }} />
        <Link href="/citizen/services/returnees" className="mt-4 inline-block rounded-md border border-slate-200/90 bg-white px-2 py-1.5 text-sm font-semibold text-[var(--gov-primary)] no-underline">العودة للخدمات الاجتماعية</Link>
      </div>
    </div>
  );
}
