...
type Props = { searchParams: Promise<{ ok?: string; no?: string }> };

export default async function CitizenReturneesOnlyPage({ searchParams }: Props) {
  const s = await auth();
  if (!s?.user) redirect("/citizen/welcome?next=/citizen/services/returnees/returnees");
  ...
  return (
    <div className="gov-panel w-full px-0">
      ...
      <div className="gov-card p-4 md:p-6">
        <ReturneeRegistrationForm action={submitReturneeRegistration} successReturnPath="/citizen/services/returnees" prefill={{ name: prefill?.name, phone: prefill?.phone, nationalId: prefill?.nationalId, email: emailDefault || null }} />
