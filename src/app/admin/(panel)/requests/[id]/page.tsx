...
export default async function AdminRequestDetailPage({ params, searchParams }: P) {
  ...
  await requireStaffPanelPermission(session, "requests");
  ...
  return (
    <div>
      {statusError && (
        ...
        >
          تعذّر حفظ تغيير الحالة. أعد المحاولة؛ إذا استمر الخطأ راجع سجلات الخادم (Render → Logs) أو اتصال قاعدة البيانات Turso.
        </p>
      )}
      ...
      <div className="max-w-3xl space-y-5">
        <div className="gov-card p-4">
          ...
          <form action={updateRequestStatus} className="max-w-md space-y-4">
            ...
            <FieldGroup>
              ...
              <Textarea name="noteForCitizen" rows={2} />
            </FieldGroup>
        ...
        <div className="gov-card p-4">
          ...
          <form action={addRequestNote} className="max-w-md space-y-3">
            <input type="hidden" name="requestId" value={r.id} />
            <input type="hidden" name="actorPortal" value="staff" />
            <Textarea name="body" required rows={3} placeholder="للإدارة والموظفين" />
        ...
        <Link href="/admin/requests" className="text-sm text-[var(--gov-primary)] hover:underline">
          ← القائمة
        </Link>
      </div>
    </div>
