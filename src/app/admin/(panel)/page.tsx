...
const adminTiles = [
  { href: "/admin/requests", title: "طلبات خدمات المدينة", sub: "عرض شامل", perm: "viewRequests" as const },
...
export default async function AdminHomePage() {
  ...
  if (s?.user?.role === UserRole.EMPLOYEE) {
    ...
    const extraTiles = adminTiles.filter((t) => perms[t.perm]);
    return (
      <AdminHomeDashboardWithSearch
        variant="employee"
        isAdmin={false}
        employeeQuickStats={perms.viewRequests ? [...cards] : []}
  ...
  return (
    <AdminHomeDashboardWithSearch
      ...
      adminTiles={
        isAdmin
          ? [
              ...adminTiles.filter((t) => perms[t.perm]).map((t) => ({ href: t.href, title: t.title, sub: t.sub })),
